...
<body style="background-color: #f0f2f5;">
	...
	<table class="table  table-hover table-dark " style="margin: 5px;">
		...
		<c:forEach var="user" items="${stulist}">
			<tr>
				<td scope="row">${user.e_id}</td>
				<td scope="row">${user.name}</td>
				<td scope="row">${user.email}</td>
				<td scope="row">${user.phone}</td>
				<td scope="row">${user.designation}</td>
  ...
  <a href="setstatus?id=${user.e_id}" class="btn btn-primary " aria-current="page"   >${user.status}</a>
  
</div></td>
				
				
				
				<td scope="row"><a href="deleteuser?id=${user.e_id}"><input
